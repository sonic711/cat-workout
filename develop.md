...
* `ExerciseManager` 對話框提供訓練動作列表、使用次數與建立 / 更新時間，支援新增、編輯、刪除（有紀錄引用時禁止刪除）。
* `useWorkoutStore` 新增動作使用統計、更新與刪除 API，並確保名稱唯一；首頁登入面板加入「管理 / 檢視訓練動作」入口。
* `WorkoutSessionEditor` 新增「每日飲食」分區，可輸入各餐飲食項目、熱量與喝水量，並將資料序列化後交由 store / 後端保存；重量與次數欄位改為下拉選單，方便快速選取常用數值。
